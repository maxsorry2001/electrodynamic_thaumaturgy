...
public class MirageEntity extends AbstractArrow {
    ...
    public MirageEntity(Level level, LivingEntity owner) {
        super(EtEntities.MIRAGE_ENTITY.get(), level);
        this.setOwner(owner);
        ...
        if(!list.isEmpty()) {
            ...
            if(castTick <= 0) {
                ...
                while (iterator.hasNext()){
                    ...
                    castTarget.getTarget().hurt(new DamageSource(Function.getHolder(level(), Registries.DAMAGE_TYPE, EtDamageType.mirage), getOwner()), castTarget.getDamage());
                }
                list = new ArrayList<>();
                castTick = 10;
            }
