...
public class CoulombDomainBeaconEntity extends AbstractArrow {
    ...
    public void tick() {
        ...
        if(liveTick % 20 == 0 && magicItem != null){
            ...
            for (LivingEntity target : list){
                ...
                target.hurt(new DamageSource(Function.getHolder(level(), Registries.DAMAGE_TYPE, EtDamageType.origin_thaumaturgy), this.getOwner()), Function.getMagicAmount(magicItem) / 2);
                Function.checkTargetEnhancement(magicItem, target);
    ...
    private void makeParticle(LivingEntity target, float xRot, float yRot){
        ...
        List<Vec3> point = Function.rotatePointsYX(Function.getCirclePoints(30, 2), xRot, yRot);
