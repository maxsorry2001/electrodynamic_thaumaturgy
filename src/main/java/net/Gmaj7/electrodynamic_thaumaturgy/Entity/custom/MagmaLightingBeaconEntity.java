        ...
        if(opentick == 20 && !level().isClientSide()){
            ...
            for (LivingEntity target : list){
                if(target != getOwner() && magicItem != null){
                    target.hurt(new DamageSource(Function.getHolder(level(), Registries.DAMAGE_TYPE, EtDamageType.origin_thaumaturgy), this.getOwner()), Function.getMagicAmount(magicItem));
