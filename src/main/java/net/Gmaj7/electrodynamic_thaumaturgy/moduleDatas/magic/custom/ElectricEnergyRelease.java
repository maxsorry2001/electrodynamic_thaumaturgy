    ...
    public void playerCast(Player livingEntity, ItemStack itemStack, MagicDefinition magicDefinition) {
        ...
        for (LivingEntity target : list){
            if(target == livingEntity) {
                livingEntity.addEffect(new MobEffectInstance(EtEffects.ELECTRIC_ELECTRIC_RELEASE.getDelegate(), (int) (Function.getResultAmount(itemStack) * 15)));
                continue;
            }
    ...
    public void mobCast(LivingEntity source, LivingEntity target, ItemStack itemStack, MagicDefinition magicDefinition) {
        ...
        for (LivingEntity livingEntity : list){
            if(livingEntity == source) {
                source.addEffect(new MobEffectInstance(EtEffects.ELECTRIC_ELECTRIC_RELEASE.getDelegate(), (int) (Function.getResultAmount(itemStack) * 15)));
                continue;
            }
            livingEntity.hurt(new DamageSource(Function.getHolder(livingEntity.level(), Registries.DAMAGE_TYPE, EtDamageType.origin_thaumaturgy), livingEntity), Function.getResultAmount(itemStack) * magicDefinition.amountRate());
