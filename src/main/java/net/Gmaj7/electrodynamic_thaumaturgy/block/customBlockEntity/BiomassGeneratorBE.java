...
import net.Gmaj7.electrodynamic_thaumaturgy.gui.menu.BiomassGeneratorMenu;
...
import net.Gmaj7.electrodynamic_thaumaturgy.init.packets.BiomassSetPacket;
...
public class BiomassGeneratorBE extends AbstractGeneratorBE implements IItemBlockEntity, MenuProvider {
    ...
    public BiomassGeneratorBE(BlockPos pos, BlockState blockState) {
        super(EtBlockEntities.BIOMASS_GENERATOR_BE.get(), pos, blockState);
    }
    ...
    protected boolean canEnergyMake() {
        if(!level.isClientSide()){
            ...
            level.setBlockAndUpdate(getBlockPos(), getBlockState().setValue(ThermalGenerator.LIT, biomassTime > 0));
