...
import net.Gmaj7.electrodynamic_thaumaturgy.entity.custom.PulsedPlasmaEntity;
...
public class PulsedPlasma extends AbstractWideMagic{
    ...
    public void playerCast(Player livingEntity, ItemStack itemStack, MagicDefinition magicDefinition) {
        PulsedPlasmaEntity pulsedPlasmaEntity = new PulsedPlasmaEntity(livingEntity, livingEntity.level(), itemStack, magicDefinition.amountRate());
    ...
    public void mobCast(LivingEntity source, LivingEntity target, ItemStack itemStack, MagicDefinition magicDefinition) {
        ...
        PulsedPlasmaEntity pulsedPlasmaEntity = new PulsedPlasmaEntity(source, source.level(), itemStack, magicDefinition.amountRate());
    ...
    private void makeParticle(ServerLevel level, LivingEntity livingEntity) {
        ...
        for (i = 0; i < polygon.size(); i++) {
            ...
            for (int j = 0; j < line.size(); j++) {
                ...
                level.sendParticles(new PointRotateParticleOption(center.toVector3f(), new Vector3f(128, 255, 128), new Vector3f(xRot, yRot, -Mth.PI / 16), 10), pos.x(), pos.y(), pos.z(), 1, 0, 0, 0, 0);
                level.sendParticles(new PointRotateParticleOption(center.toVector3f(), new Vector3f(128, 255, 128), new Vector3f(xRot, yRot, -Mth.PI / 16), 10), pos2.x(), pos2.y(), pos2.z(), 1, 0, 0, 0, 0);
            }
        }
    }
}
