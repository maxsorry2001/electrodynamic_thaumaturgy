...
public class EntityCloneBlockMenu extends AbstractContainerMenu {
    private final Level level;
    private final int slotNum = 0;
    public  final BioReplicationVatBE blockEntity;

    public EntityCloneBlockMenu(int containerId, Inventory inventory, FriendlyByteBuf buf){
        this(containerId, inventory, inventory.player.level().getBlockEntity(buf.readBlockPos()));
    }

    public EntityCloneBlockMenu(int containerId, Inventory inventory, BlockEntity blockEntity) {
        super(EtMenuTypes.BIO_REPLICATION_VAT_MACHINE_MENU.get(), containerId);
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, EtBlocks.BIO_REPLICATION_VAT_MACHINE.get());
    }
