...
public class MagneticDissolverBlockMenu extends AbstractContainerMenu {
    private final Level level;
    private final int inNum = 0;
    private final int outNum = 1;
    private ContainerData data;
    public  final MagneticDissolverBE blockEntity;
    public MagneticDissolverBlockMenu(int containerId, Inventory inventory, FriendlyByteBuf buf){
        this(containerId, inventory, inventory.player.level().getBlockEntity(buf.readBlockPos()));
    }

    public MagneticDissolverBlockMenu(int containerId, Inventory inventory, BlockEntity blockEntity) {
        super(EtMenuTypes.MAGNETIC_DISSOLVER_BLOCK_MENU.get(), containerId);
        this.blockEntity = (MagneticDissolverBE) blockEntity;
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(level, blockEntity.getBlockPos()), player, EtBlocks.MAGNETIC_DISSOLVER_MACHINE.get());
    }
