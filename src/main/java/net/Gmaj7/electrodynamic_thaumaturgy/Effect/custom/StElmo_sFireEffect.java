...
public class StElmo_sFireEffect extends MobEffect {
    ...
    public boolean applyEffectTick(ServerLevel serverLevel, LivingEntity livingEntity, int amplification) {
        ...
        for (LivingEntity target : list){
            if (target != livingEntity && target instanceof Enemy){
                target.hurt(new DamageSource(Function.getHolder(livingEntity.level(), Registries.DAMAGE_TYPE, EtDamageType.origin_thaumaturgy), livingEntity), 5 + 2F * amplification);
    ...
    private void makeParticle(ServerLevel level, LivingEntity source, LivingEntity target){
        ...
        List<Vec3> points = Function.getLinePoints(start, end, 30);
