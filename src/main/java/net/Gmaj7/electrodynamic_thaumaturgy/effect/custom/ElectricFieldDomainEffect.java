...
public class ElectricFieldDomainEffect extends MobEffect {
    ...
    public boolean applyEffectTick(ServerLevel serverLevel, LivingEntity livingEntity, int amplifier) {
        ...
        for (LivingEntity target : list){
            if(target != livingEntity && target.walkAnimation.speed() > 0.01 && target.onGround()) {
                ...
                rr = Math.max(rr, 1);
                target.hurt(new DamageSource(Function.getHolder(livingEntity.level(), Registries.DAMAGE_TYPE, EtDamageType.origin_thaumaturgy), livingEntity), num / rr);
            }
        }
    ...
    private void makeParticle(ServerLevel serverLevel, LivingEntity livingEntity){
        List<Vec3> point = Function.rotatePointsYX(Function.getCirclePoints(60, 10), Mth.PI / 2, 0);
