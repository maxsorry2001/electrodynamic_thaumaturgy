...
public class PhotoCorrosiveNovaEntity extends Entity {
    ...
    public void tick() {
        ...
        if(!this.level().getBlockState(this.blockPosition()).is(EtBlocks.LIGHT_AIR)){
            this.level().setBlockAndUpdate(this.blockPosition(), EtBlocks.LIGHT_AIR.get().defaultBlockState());
        }
        if(this.tickCount % 20 == 0){
            ...
            for (LivingEntity target : list)
                target.addEffect(new MobEffectInstance(EtEffects.PHOTO_CORROSIVE, 100, amplifier));
        }
