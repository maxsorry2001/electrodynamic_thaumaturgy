...
public class FrequencyDivisionArrowEntity extends Arrow {
    ...
    public void tick() {
        ...
        if(explodeTime <= 0 && magicItem != null){
            ...
            for (LivingEntity target : list)
                target.hurt(new DamageSource(MoeFunction.getHolder(level(), Registries.DAMAGE_TYPE, MoeDamageType.origin_thaumatury), getOwner()), MoeFunction.getMagicAmount(magicItem) / 7);
            this.discard();
        }
    }
    ...
    protected void onHitEntity(EntityHitResult result) {
        ...
        if(entity instanceof LivingEntity livingEntity && livingEntity != getOwner()){
            if(magicItem != null)
                livingEntity.hurt(new DamageSource(MoeFunction.getHolder(level(), Registries.DAMAGE_TYPE, MoeDamageType.origin_thaumatury), getOwner()), MoeFunction.getMagicAmount(magicItem));
            this.discard();
        }
    }
